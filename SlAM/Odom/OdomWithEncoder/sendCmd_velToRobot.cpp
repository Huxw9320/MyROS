/*　base_controller节点将接收到的cmd_vel速度信息转换为自定义的结构体或union类型的数据（自定义的数据类型中可以包含校验码等其它信息），
并通过串口发送控制速度信息（speed_buf）或读取机器人传回的速度信息 (speed_buf_rev)。base_controller节点正确读取到底层（比如嵌入式控制板）传回的速度后进行积分，
计算出机器人的估计位置和姿态，并将里程计信息和tf变换发布出去。下面的代码只是一个例子，需要完善自定义的数据结构和校验函数：*/
